...
本仓库当前的 Native 构建流程统一使用 `python3 scripts/build-native.py`。
...
- Native 平台（Linux / macOS / Windows）统一使用 `python3 scripts/build-native.py`
...
- `scripts/build-native.py` 会自动查找 Visual Studio 并调用 `vcvars64.bat`
...
```bash
python3 scripts/build-native.py --config debug
```

### Linux 上切换编译器

```bash
python3 scripts/build-native.py --compiler gcc
python3 scripts/build-native.py --compiler clang --config debug --test
```
...
```bash
python3 scripts/build-native.py --preset windows-msvc-debug
```

### 列出可用 preset

```bash
python3 scripts/build-native.py --list-presets
```

### 清理后重建

```bash
python3 scripts/build-native.py --clean --test -j 8
```
...
- `scripts/build-native.py --clean` 会在 configure 前清理对应的构建目录
